<x-home.layout title="Transaksi">
    <div class="min-h-[55vh] px-5 my-24 md:my-20 md:px-10">
                                ...
                                <td class="flex items-center justify-center gap-2 p-3">
                                    ...
                                    @elseif ($transaction->status == 'processing' || $transaction->status == 'completed')
                                            <button type="button" data-modal-target="rating-modal"
                                                ...
                                                {{ \Carbon\Carbon::parse($transaction->tgl_booking)->lt(now()) ? '' : 'disabled' }}
                                                class="text-yellow-400 hover:text-white border border-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:outline-none focus:ring-yellow-50 font-medium rounded-lg text-sm px-4 py-1.5 text-center me-2 mb-2 disabled:opacity-50 disabled:cursor-not-allowed">
                                                Review & Rating
                                            </button>
                                    @endif
                                </td>
...
@include('components.modal.user.rating-review', ['cafe_id' => $transaction->cafe_id])
